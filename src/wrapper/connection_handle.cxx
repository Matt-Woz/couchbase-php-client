                                        ...
                                        const zval* options)
{
    ...
    auto agent_group = couchbase::core::agent_group(cluster_->io_context(), couchbase::core::agent_group_config{ { cluster_ } });
    agent_group.open_bucket(bucket_name);
    agent = agent_group.get_agent(bucket_name);
    if (!agent.has_value()) {
    ...
    if (!vbucket_map || vbucket_map->empty()) {
        return { std::errc{}, ERROR_LOCATION, "Cannot perform scan operation. Unable to get vBucket map." };
    }
    ...
    auto orchestrator = couchbase::core::range_scan_orchestrator(
        cluster_->io_context(), agent.value(), vbucket_map.value(), scope_name, collection_name, core_scan_type, opts);
    ...
    zend_class_entry* core_scan_result_ce = couchbase::php::core_scan_result();
    object_init_ex(return_value, core_scan_result_ce);
    zend_update_property(core_scan_result_ce, Z_OBJ_P(object), ZEND_STRL("core"), connection);
    cb_core_scan_result_data* intern;
    intern = zend_object_store_get_object(return_value);
    intern->scan_result = std::make_unique<couchbase::core::scan_result>(resp.value());
